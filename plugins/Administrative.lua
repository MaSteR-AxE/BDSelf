...
if ((mr_roo[1]:lower() == "sendplug") or (mr_roo[1] == "ارسال پلاگین")) and mr_roo[2] then
	...
	tdbot.sendDocument(msg.to.id, plug, M_START..""..EndPm, nil, msg.id, 0, 1, nil, dl_cb, nil)
end
end
--######################################################################--
...
--######################################################################--
if ((mr_roo[1]:lower() == "remsudo") or (mr_roo[1] == "حذف سودو"))   then
	if not mr_roo[2] and msg.reply_id then
		tdbot_function ({
		_ = "getMessage",
		chat_id = msg.to.id,
...
if ((mr_roo[1]:lower() == "config") or (mr_roo[1] == "پیکربندی")) and is_admin(msg)   then
	return set_config(msg)
end
--######################################################################--
if (matches[1]:lower() == "testspeed" or matches[1] == "سرعت سرور") and is_admin(msg)   then
...
--######################################################################--
if (mr_roo[1]:lower() == "panelsudo" or mr_roo[1] == "پنل سودو") and is_admin(msg)   then
	local function inline_query_cb(arg, data)
		if data.results and data.results[0] then
			...
			text = M_START.."𝒯𝑒𝒸𝒽𝓃𝒾𝒸𝒶𝓁 𝓅𝓇𝑜𝒷𝓁𝑒𝓂 𝒾𝓃 𝒶𝓊𝓍𝒾𝓁𝒾𝒶𝓇𝓎 𝓇𝑜𝒷𝑜𝓉"..EndPm
...
if ((mr_roo[1]:lower() == 'leave') or (mr_roo[1] == "خروج")) and mr_roo[2]   then
	tdbot.sendMessage(mr_roo[2], 0, 1, M_START..'ربات با دستور سازنده از گروه خارج شد .\nبرای اطلاعات بیشتر با سازنده تماس بگیرید .'..EndPm..'\n` سازنده ربات :` '..check_markdown(sudo_username), 1, 'md')
	tdbot.changeChatMemberStatus(mr_roo[2], our_id, 'Left', dl_cb, nil)
	tdbot.sendMessage(gp_sudo, msg.id, 1, M_START..'ربات با موفقیت از گروه '..mr_roo[2]..' خارج شد.'..EndPm..'\nتوسط : '..check_markdown(msg.from.print_name or '')..' | `'.. msg.from.print_name..'`', 1,'md')
end
--######################################################################--
if ((mr_roo[1]:lower() == 'charge') or (mr_roo[1] == "شارژ")) and mr_roo[2] and mr_roo[3]   then
	if string.match(mr_roo[2], '^-%d+$') then
		if tonumber(mr_roo[3]) > 0 and tonumber(mr_roo[3]) < 1001 then
			...
			tdbot.sendMessage(msg.to.id, msg.id, 1, M_START..'*𝒯𝒽𝑒 𝓃𝓊𝓂𝒷𝑒𝓇 𝑜𝒻 𝒹𝒶𝓎𝓈 𝓈𝒽𝑜𝓊𝓁𝒹 𝒷𝑒 𝒷𝑒𝓉𝓌𝑒𝑒𝓃 1 𝒶𝓃𝒹 1000*'..EndPm, 1, 'md')
		end
	end
end
--######################################################################--
...
if ((mr_roo[1]:lower() == 'jointo') or (mr_roo[1] == "ورود به")) and mr_roo[2]   then
	if string.match(mr_roo[2], '^-%d+$') then
		...
		tdbot.sendMessage(mr_roo[2], 0, 1, M_START..'* سازنده ربات به گروه اضافه شد*'..EndPm..'\n`سازنده :` '..check_markdown(sudo_username), 1, 'md')
	end
end
--######################################################################--
end
if msg.to.type == 'channel' or msg.to.type == 'chat' then
if ((mr_roo[1]:lower() == 'charge') or (mr_roo[1] == "شارژ")) and mr_roo[2] and not mr_roo[3] and is_sudo(msg)   then
	local linkgp = data[tostring(chat)]['settings']['linkgp']
	if not linkgp then
		return M_START..'`لطفا قبل از شارژ گروه لینک گروه را تنظیم کنید`'..EndPm..'\n*"تنظیم لینک"\n"setlink"*'
	end
...
--######################################################################--	
if ((mr_roo[1]:lower() == 'expire') or (mr_roo[1] == "اعتبار")) and is_mod(msg) and not mr_roo[2]   then
...
if ((mr_roo[1]:lower() == 'expire') or (mr_roo[1] == "اعتبار")) and is_mod(msg) and mr_roo[2]   then
	if string.match(mr_roo[2], '^-%d+$') then
...
--######################################################################--
if ((mr_roo[1]:lower() == "setadmin") or (mr_roo[1] == "تنظیم ادمین")) and is_sudo(msg)   then
	if not mr_roo[2] and msg.reply_id then
		tdbot_function ({
		_ = "getMessage",
		chat_id = msg.to.id,
...
--######################################################################--
if ((mr_roo[1] == "remadmin") or (mr_roo[1] == "حذف ادمین")) and is_sudo(msg)   then
	if not mr_roo[2] and msg.reply_id then
		tdbot_function ({
		_ = "getMessage",
		chat_id = msg.to.id,
...
--######################################################################--
if ((mr_roo[1]:lower() == 'creategroup') or (mr_roo[1] == "ساخت گروه")) and is_admin(msg)   then
	local text = mr_roo[2]
	tdbot.createNewGroupChat({[0] = msg.from.id}, text, dl_cb, nil)
	return M_START..'`گروه ساخته شد`'..EndPm
end
--######################################################################--
if ((mr_roo[1]:lower() == 'createsuper') or (mr_roo[1] == "ساخت سوپرگروه")) and is_admin(msg)   then
local text = mr_roo[2]
tdbot.createNewChannelChat(text, 1, 'poinshtan bot', (function(b, d) tdbot.addChatMember(d.id, msg.from.id, 0, dl_cb, nil) end), nil)
	return M_START..'*سوپرگروه ساخته شد و* [`'..msg.from.id..'`] *به گروه اضافه شد.*'..EndPm
end
--######################################################################--
if ((mr_roo[1]:lower() == 'tosuper') or (mr_roo[1] == "تبدیل به سوپرگروه")) and is_admin(msg)   then
	local id = msg.to.id
	tdbot.migrateGroupChatToChannelChat(id, dl_cb, nil)
	return M_START..'`گروه به سوپر گروه تبدیل شد`'..EndPm
end
--######################################################################--
if ((mr_roo[1]:lower() == 'import') or (mr_roo[1] == "ورود لینک")) and is_admin(msg)   then
...
--######################################################################--
if ((mr_roo[1]:lower() == 'setbotname') or (mr_roo[1] == "تغییر نام ربات")) and is_sudo(msg)   then
	tdbot.changeName(mr_roo[2], dl_cb, nil)
	return M_START..'`اسم ربات تغییر کرد به :`\n*'..mr_roo[2]..'*'..EndPm
end
--######################################################################--
if ((mr_roo[1]:lower() == 'setbotusername') or (mr_roo[1] == "تغییر یوزرنیم ربات")) and is_sudo(msg)   then
	tdbot.changeUsername(mr_roo[2], dl_cb, nil)
	return M_START..'`یوزرنیم ربات تغییر کرد به :` \n@'..check_markdown(mr_roo[2])..''..EndPm
end
--######################################################################--
if ((mr_roo[1]:lower() == 'delbotusername') or (mr_roo[1] == "حذف یوزرنیم ربات")) and is_sudo(msg)   then
	tdbot.changeUsername('', dl_cb, nil)
	return M_START..'*انجام شد*'..EndPm
end
--######################################################################--
if ((mr_roo[1]:lower() == 'markread') or (mr_roo[1] == "تیک دوم")) and is_sudo(msg)   then
...
--######################################################################--
if ((mr_roo[1]:lower() == 'bc') or (mr_roo[1] == "ارسال")) and is_admin(msg)   then
	local text = mr_roo[2]
	tdbot.sendMessage(mr_roo[3], "", 0, text, 0,  "html")
end
--######################################################################--
if ((mr_roo[1]:lower() == 'broadcast') or (mr_roo[1] == "ارسال به همه")) and is_sudo(msg)   then
	local data = load_data(_config.moderation.data)
	local bc = mr_roo[2]
...
--######################################################################--
if ((mr_roo[1]:lower() == 'sudolist') or (mr_roo[1] == "لیست سودو")) and is_sudo(msg)   then
	return sudolist(msg)
end
--######################################################################--
if ((mr_roo[1]:lower() == 'chats') or (mr_roo[1] == "لیست گروه ها")) and is_admin(msg)   then
	return chat_list(msg)
end
--######################################################################--
if ((mr_roo[1]:lower() == 'join') or (mr_roo[1] == "ورود")) and is_admin(msg) and mr_roo[2]   then
	tdbot.sendMessage(msg.to.id, msg.id, 1, M_START..'*شما وارد گروه * '..mr_roo[2]..' *شدید*'..EndPm, 1, 'md')
	tdbot.sendMessage(mr_roo[2], 0, 1, M_START.."*سازنده ربات وارد گروه شد 🌹*"..EndPm, 1, 'md')
	tdbot.addChatMember(mr_roo[2], msg.from.id, 0, dl_cb, nil)
end
--######################################################################--
if ((mr_roo[1]:lower() == 'rem') or (mr_roo[1] == "حذف گروه")) and mr_roo[2] and is_admin(msg)   then
	local data = load_data(_config.moderation.data)
...
--######################################################################--
if ((mr_roo[1]:lower() == 'adminlist') or (mr_roo[1] == "لیست ادمین")) and is_admin(msg)   then
	return adminlist(msg)
end
--######################################################################--
if ((mr_roo[1]:lower() == 'leave') or (mr_roo[1] == "خروج")) and not mr_roo[2] and is_admin(msg)   then
...
--######################################################################--
if ((mr_roo[1]:lower() == 'autoleave') or (mr_roo[1] == "خروج خودکار")) and is_admin(msg)   then
	...
	elseif ((mr_roo[2] == 'disable') or (mr_roo[2] == "غیرفعال")) then
		...
		return M_START..'𝒜𝓊𝓉𝑜𝓈𝒾𝑔𝓃𝒶𝓁 𝒹𝒾𝓈𝒶𝒷𝓁𝑒𝒹'..EndPm
	end
end
--######################################################################--
if (mr_roo[1]:lower() == 'panelgp' or mr_roo[1] == 'پنل گروه') and is_admin(msg)   then
	local function inline_query_cb(arg, data)
		if data.results and data.results[0] then
			redis:setex(RedisIndex.."ReqMenu:" .. msg.to.id .. ":" .. msg.from.id, 260, true) redis:setex(RedisIndex.."ReqMenu:" .. msg.to.id, 10, true) tdbot.sendInlineQueryResultMessage(msg.to.id, msg.id, 0, 1, data.inline_query_id, data.results[0].id, dl_cb, nil)
		else
...
--######################################################################--
if (mr_roo[1]:lower() == 'codegift' or mr_roo[1] == 'کدهدیه') and is_sudo(msg)   then
	local code = {'1','2','3','4','5','6','7','8','9','a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'}
	local charge = {2,5,8,10,11,14}
	...
	local codetext = "POINSHTAN-"..a..b..c..d..e..f.."-BOT"
...
--######################################################################--
if (mr_roo[1]:lower() == 'giftlist' or mr_roo[1] == 'لیست کدهدیه') and is_sudo(msg)   then
	local list = redis:smembers(RedisIndex.."CodeGift:")
	local text = '𝒞𝑜𝒹𝑒 𝓁𝒾𝓈𝓉 𝑜𝒻 𝓂𝒶𝒹𝑒 𝑔𝒾𝒻𝓉𝓈 :\n'
	...
	if #list == 0 then
	text = M_START..'𝒩𝑜 𝑔𝒾𝒻𝓉 𝒸𝑜𝒹𝑒 𝒽𝒶𝓈 𝒷𝑒𝑒𝓃 𝒸𝓇𝑒𝒶𝓉𝑒𝒹'..EndPm
	end
	tdbot.sendMessage(msg.chat_id, msg.id, 1, text, 1, 'md')
end
--######################################################################--
if (mr_roo[1]:lower() == 'gift' or mr_roo[1] == 'استفاده هدیه') and is_owner(msg)   then
