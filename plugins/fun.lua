...
--------------------------------
	if (matches[1]:lower() == 'praytime') or (matches[1]:lower() == 'ساعات شرعی' ) then
		if matches[2] then
			city = matches[2]
			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, '\n🦋 AxE 🌹', dl_cb, nil)
					else
						tdcli.sendMessage(msg.to.id, msg.id_, 1, '_This sticker does not exist. Send sticker again._'..'\n🦋 AxE 🌹', 1, 'md')
					end
				else
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '\n🦋 AxE 🌹', dl_cb, nil)
					else
...
--------------------------------
 ...
 local text = matches[2]
  ...
  local file = download_to_file(url,'MaSteR-AxE.ogg')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, matches[2]..'\n🦋 AxE 🌹', dl_cb, nil)
   end
end
...
if matches[1] == "راهنمای سرگرمی" then
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not lang then
helpfun_fa = [[
