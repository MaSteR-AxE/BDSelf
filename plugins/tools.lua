--Begin Tools.lua :)
local SUDO = 776400879 -- put Your ID here! <===
...
if is_sudo(msg) then
   		if ((matches[1]:lower() == 'add'  ) or (matches[1] == "افزودن"  )) and not redis:get('ExpireDate:'..msg.to.id) then
			redis:set('ExpireDate:'..msg.to.id,true)
...
if ((matches[1] == 'markread'  ) or (matches[1] == ""  )) and is_sudo(msg) then
...
tdcli.sendMessage(k, 0, 0, bc, 0)			
  ...
  if is_sudo(msg) then
	if ((matches[1]:lower() == "sendfile"  ) or (matches[1] == "ارسال فایل"  )) and matches[2] and matches[3] then
		local sendfile = "./"..matches[2].."/"..matches[3]
		tdcli.sendDocument(msg.to.id, msg.id,0, 1, nil, sendfile, '🦋 AxE 🌹', dl_cb, nil)
	end
	if ((matches[1]:lower() == "sendplug"  ) or (matches[1] == "ارسال پلاگین"  )) and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		tdcli.sendDocument(msg.to.id, msg.id,0, 1, nil, plug, '🦋 AxE 🌹', dl_cb, nil)
    end
  end
